...
function createModuleResolver(ts, host, filter) {
	...
	return (moduleName, containingFile, redirectedReference, mode) => {
		 ...
		 * */
		if (
			(resolvedModule === null || resolvedModule === void 0
				? void 0
...
function makePathsAbsolute(compilerOptions, relativeTo) {
	for (const pathProp of DIRECTORY_PROPS) {
		if (compilerOptions[pathProp]) {
			compilerOptions[pathProp] = resolve$1(
				relativeTo,
...
function makeForcedCompilerOptions(noForceEmit) {
	return {
		...
		...(noForceEmit ? {} : OVERRIDABLE_EMIT_COMPILER_OPTIONS)
	};
}
/**
...
async function emitFile(
	...
	if (relativePath.startsWith('..')) {
		...
		if (outputToFilesystem !== false) {
			await promises.mkdir(path.dirname(normalizedFilePath), {
				recursive: true
			});
...
function createWatchHost(
	ts,
	context,
	{ formatHost, parsedOptions, writeFile, status, resolveModule, transformers }
) {
		...
		afterProgramCreate(program) {
			...
			program.emit = (targetSourceFile, _, ...args) =>
				origEmit(
					...
					// emitOnlyDtsFiles
					args[1],
					mergeTransformers(program, transformers, args[2])
				);
		...
		resolveModuleNames(
			...
			return moduleNames.map((moduleName, i) => {
				...
				const mode = containingSourceFile
					? (_a = ts.getModeForResolutionAtIndex) === null || _a === void 0
						? void 0
						: _a.call(ts, containingSourceFile, i)
...
class TSCache {
	...
	getCached(fileName) {
		...
		if (this.isCached(fileName)) {
			code = fs.readFileSync(this.cachedFilename(fileName), {
				encoding: 'utf-8'
			});
		}
		return code;
...
function typescript(options = {}) {
	...
	const parsedOptions = parseTypescriptConfig(
		ts,
		tsconfig,
		compilerOptions,
		noForceEmit
	...
	const filter = createFilter(include || '{,**/}*.(cts|mts|ts|tsx)', exclude, {
		resolve:
			filterRoot !== null && filterRoot !== void 0
				? filterRoot
	...
	return {
		...
		buildStart(rollupOptions) {
			...
			preflight({
				...
				inputPreserveModules: rollupOptions.preserveModules,
				tslib
			});
			...
			if (!program) {
				program = createWatchProgram(ts, this, {
					...
					writeFile(fileName, data) {
						if (
							parsedOptions.options.composite ||
							parsedOptions.options.incremental
						) {
		...
		async generateBundle(outputOptions) {
			const declarationAndTypeScriptMapFiles = [...emittedFiles.keys()].filter(
				(fileName) =>
					isDeclarationOutputFile(fileName) ||
					isTypeScriptMapOutputFile(fileName)
			);
