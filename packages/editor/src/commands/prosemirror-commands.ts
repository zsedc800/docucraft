/**
 * fork from prosemirror-commands enhance nodeView
 */
import {
	joinPoint,
	canJoin,
...
export const exitCode: Command = (state, dispatch) => {
	...
	if (dispatch) {
		let pos = $head.after(),
			tr = state.tr.replaceWith(pos, pos, createNodeAndFill(type)!);
...
export const createParagraphNear: Command = (state, dispatch) => {
	...
	if (dispatch) {
		...
		let tr = state.tr.insert(side, createNodeAndFill(type)!);
