...
export function onTarget(editor: IEditor, oldValue: IUI | IUI[]): void {
	...
	if (target) {
		editor.leafList =
			target instanceof LeafList
				? target
				: new LeafList(target instanceof Array ? target : (target as IUI));
	...
	editor.emitEvent(
		new EditorEvent(EditorEvent.SELECT, { editor, value: target, oldValue })
	);
...
export function onHover(editor: IEditor, oldValue: IUI): void {
	editor.emitEvent(
		new EditorEvent(EditorEvent.HOVER, {
			editor,
