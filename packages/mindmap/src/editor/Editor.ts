...
export class Editor extends Group implements IEditor {
	...
	public get mergeConfig(): IEditorConfig {
		...
		return this.single && element.editConfig
			? { ...config, ...element.editConfig }
			: config; // 实时合并，后期可优化
	}
	...
	public get list(): IUI[] {
		return this.leafList.list as IUI[];
	}
	public get dragHoverExclude(): IUI[] {
	...
	public get groupOpening(): boolean {
	...
	public get element() {
		return this.multiple
			? this.simulateTarget
			: (this.list[0] as ISimulateElement);
	}
	...
	public addItem(item: IUI): void {
		if (!this.hasItem(item) && !item.locked)
			this.leafList.add(item), (this.target = this.leafList.list as IUI[]);
	}

	public removeItem(item: IUI): void {
		if (this.hasItem(item))
			this.leafList.remove(item), (this.target = this.leafList.list as IUI[]);
	}
	...
	public updateEditTool(): void {
		...
		if (this.editing) {
			...
			this.editTool = this.editToolList[tag] =
				this.editToolList[tag] || EditToolCreator.get(tag, this);
			this.editBox.load();
			this.editTool.load();
		}
	}
	...
	public onMove(e: DragEvent | MoveEvent): void {
		if (e instanceof MoveEvent) {
			...
			this.move(
				DragEvent.getValidMove(this.element, this.dragStartPoint, total)
			);
		}
	}
	...
	public onRotate(e: DragEvent | RotateEvent): void {
		...
		if (e instanceof RotateEvent) {
			...
			const data = EditDataHelper.getRotateData(
				element.boxBounds,
				direction,
				e.getBoxPoint(element),
				element.getBoxPoint(last),
				e.shiftKey
					? null
					: element.around || element.origin || around || 'center'
			);
	...
	public onSkew(e: DragEvent): void {
		...
		const { origin, skewX, skewY } = EditDataHelper.getSkewData(
			element.boxBounds,
			(e.current as IEditPoint).direction,
			e.getInnerMove(element),
			EditDataHelper.getAround(around, e.altKey)
		);
	...
	public move(x: number | IPointData, y = 0): void {
		...
		const world = element.getWorldPointByLocal(
			typeof x === 'object' ? { ...x } : { x, y },
			null,
			true
		);
	...
	public scaleWithDrag(data: IEditorScaleEvent): void {
		...
		const { element } = this;
		const event = new EditorScaleEvent(EditorScaleEvent.SCALE, {
			...data,
			target: element,
			editor: this,
	...
	override scaleOf(
		...
		const transform =
			this.multiple &&
			this.getChangedTransform(() =>
				element.safeChange(() => element.scaleOf(origin, scaleX, scaleY))
			);
		const event = new EditorScaleEvent(EditorScaleEvent.SCALE, {
			target: element,
			editor: this,
	...
	override flip(axis: IAxis): void {
		...
		const transform = this.multiple
			? this.getChangedTransform(() =>
					element.safeChange(() => element.flip(axis))
				)
			: new Matrix(LeafHelper.getFlipTransform(element, axis));
		const event = new EditorScaleEvent(EditorScaleEvent.SCALE, {
			target: element,
			editor: this,
			worldOrigin,
	...
	override rotateOf(origin: IPointData | IAlign, rotation: number): void {
		...
		const transform =
			this.multiple &&
			this.getChangedTransform(() =>
				element.safeChange(() => element.rotateOf(origin, rotation))
			);
	...
	override skewOf(
		...
		const transform =
			this.multiple &&
			this.getChangedTransform(() =>
				element.safeChange(() => element.skewOf(origin, skewX, skewY))
			);
	...
	public checkTransform(
		...
		return (
			this.element &&
			!this.element.locked &&
			(this.mergeConfig[type] as boolean)
		);
	}

	protected getWorldOrigin(origin: IPointData | IAlign): IPointData {
		return this.element.getWorldPoint(
			LeafHelper.getInnerOrigin(this.element, origin)
		);
	}
	...
	public ungroup(): IUI[] {
		...
		if (list.length) {
			list.forEach(
				(item) =>
					item.isBranch &&
					this.emitGroupEvent(EditorGroupEvent.BEFORE_UNGROUP, item as IGroup)
			);
			this.target = EditorHelper.ungroup(list);
			list.forEach(
				(item) =>
					item.isBranch &&
					this.emitGroupEvent(EditorGroupEvent.UNGROUP, item as IGroup)
			);
		}
		return this.list;
	}
		...
		if (opened.length) {
			...
			if (this.editing)
				(list = []),
					opened.forEach(
						(item) =>
							this.list.every((leaf) => !LeafHelper.hasParent(leaf, item)) &&
							list.push(item)
					);
	...
	public listenTargetEvents(): void {
		if (!this.targetEventIds.length) {
			...
			this.targetEventIds = [
				...
				app.on_([KeyEvent.HOLD, KeyEvent.UP], (e: KeyEvent) => {
					updateCursor(this, e);
				}),
