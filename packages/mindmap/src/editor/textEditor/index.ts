...
import { registerInnerEditor } from '../tool/EditToolCreator';
import { InnerEditor } from '../tool/InnerEditor';
...
export class TextEditor extends InnerEditor {
	...
	public onLoad(): void {
		...
		this.isHTMLText
			? (div.innerHTML = String(text.text))
			: (div.innerText = String(text.text));

		const { view } = editor.app;
		(this.inBody = view instanceof HTMLCanvasElement)
			? document.body.appendChild(div)
			: (view as HTMLDivElement).appendChild(div);
	...
	public onUpdate() {
		...
		// layout
		const { a, b, c, d, e, f } = new Matrix(text.worldTransform).scale(
			1 / textScale
		);
