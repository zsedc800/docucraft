...
import { IEditor, IStroker } from '@leafer-in/interface';
...
export class Stroker extends UI implements IStroker {
	...
	public __draw(canvas: ILeaferCanvas, options: IRenderOptions): void {
		const {
			list,
			editor: {
				mergeConfig: { selectorPadding: padding }
			}
		} = this;
		...
		if (list.length) {
			...
			for (let i = 0; i < list.length; i++) {
				...
				if (!bounds || bounds.hit(worldRenderBounds, options.matrix)) {
					const aScaleX = abs(worldTransform.scaleX),
						aScaleY = abs(worldTransform.scaleY);

					if (aScaleX !== aScaleY) {
						// need no scale stroke, use rect path
						...
						if (leaf.__.__useArrow) leaf.__drawPath(canvas);
							...
							leaf.__.__pathForRender
								? leaf.__drawRenderPath(canvas)
								: // : leaf.__drawPathByBox(canvas);
									canvas.roundRect(
										x - padding,
										y - padding,
										width + 2 * padding,
										height + 2 * padding,
										leaf.cornerRadius as number
									);
					...
					if (stroke)
						typeof stroke === 'string'
							...
							: Paint.strokes(stroke, this, canvas);
					if (fill)
						typeof fill === 'string'
							? Paint.fill(fill, this, canvas)
							: Paint.fills(fill, this, canvas);
				}
			}
