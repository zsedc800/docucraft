...
export class EditSelect extends Group implements IEditSelect {
	...
	protected onHover(): void {
		...
		if (this.running && !this.dragging && !editor.dragging) {
			...
			this.hoverStroker.setTarget(hover ? this.editor.hoverTarget : null, {
				stroke,
				strokeWidth,
				...(hoverStyle || {})
			});
	...
	protected onPointerMove(e: PointerEvent): void {
		...
		if (
			this.running &&
			!this.isMoveMode &&
			app.interaction.canHover &&
			!app.interaction.dragging
		) {
			...
			if (find) {
				if (this.isMultipleSelect(e)) {
					if (editor.hasItem(find))
						this.needRemoveItem = find; // 等待tap事件再实际移除
	...
	protected onDrag(e: DragEvent): void {
		...
		if (this.dragging) {
			...
			if (list.length) {
				...
				if (
					selectList.length !== editor.list.length ||
					editor.list.some((child, index) => child !== selectList[index])
				) {
	...
	protected allowDrag(e: DragEvent) {
		if (
			this.running &&
			this.editor.mergeConfig.boxSelect &&
			!e.target.draggable
		) {
			return (
				(!this.editor.editing && this.allow(e.target)) ||
				(e.shiftKey && !findOne(e.path))
			);
		} else {
			return false;
		}
	...
	protected __listenEvents(): void {
		...
		editor.waitLeafer(() => {
			...
			this.__eventIds = [
				...
				app.on_([ZoomEvent.ZOOM, MoveEvent.MOVE], () => {
