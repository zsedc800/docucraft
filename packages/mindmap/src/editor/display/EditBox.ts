...
export class EditBox extends Group implements IEditBox {
	...
	public create() {
		let rotatePoint: IEditPoint,
			resizeLine: IEditPoint,
			resizePoint: IEditPoint;
		const {
			view,
			resizePoints,
			rotatePoints,
	...
	public load(): void {
		...
		for (let i = 0; i < 8; i++) {
			...
			resizeP.set(
				this.getPointStyle(
					i % 2
						? middlePointsStyle[((i - 1) / 2) % middlePointsStyle.length]
						: pointsStyle[(i / 2) % pointsStyle.length]
				)
			);
		...
		circle.set(
			this.getPointStyle(
				mergeConfig.circle || mergeConfig.rotatePoint || pointsStyle[0]
			)
		);
	...
	public update(bounds: IBoundsData): void {
		...
		if (this.view.worldOpacity) {
			...
			const { rect, circle, buttons, resizePoints, rotatePoints, resizeLines } =
				this;
			...
			circle.visible =
				showPoints &&
				rotateable &&
				!!(mergeConfig.circle || mergeConfig.rotatePoint);
	...
	protected layoutCircle(config: IEditorConfig): void {
		...
		const direction = fourDirection.indexOf(
			circleDirection ||
				(this.buttons.children.length && buttonsDirection === 'bottom'
					? 'top'
					: 'bottom')
		);
	...
	protected layoutButtons(config: IEditorConfig): void {
		...
		const direction = buttonsFixed
			? EditDataHelper.getRotateDirection(
					index,
					this.flippedOne ? this.rotation : -this.rotation,
					4
				)
			: index;
	...
	protected setButtonPosition(
		...
		const margin =
			(buttonsMargin +
				(useWidth
					? (useMiddlePoint ? point.width : 0) + buttons.boxBounds.width
					: (useMiddlePoint ? point.height : 0) + buttons.boxBounds.height) /
					2) *
			sign;
	...
	public getPointStyle(userStyle?: IBoxInputData): IBoxInputData {
		const { stroke, strokeWidth, pointFill, pointSize, pointRadius } =
			this.editor.mergeConfig;
		const defaultStyle = {
			fill: pointFill,
			stroke,
			strokeWidth,
	...
	public getMiddlePointsStyle(): IBoxInputData[] {
		...
		return middlePoint instanceof Array
			? middlePoint
			: middlePoint
				? [middlePoint]
				: this.getPointsStyle();
	...
	protected onDragStart(e: DragEvent): void {
		...
		if (e.current.name === 'rect') {
			...
			editor.dragStartBounds = {
				...editor.element.getLayoutBounds('box', 'local')
			};
		...
		if (editor.single) {
			...
			if (element.isBranch && !element.editInner) {
				if ((element as IBox).textBox) {
					...
					const find =
						children.find((item) => item.editable && item instanceof Text) ||
						children.find((item) => item instanceof Text);
	...
	public listenPointEvents(
		...
		if (point.name !== 'circle')
			point.on_(PointerEvent.ENTER, (e) => {
				(this.enterPoint = point), updateCursor(editor, e);
			});
	}
